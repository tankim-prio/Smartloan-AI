...
# ---------------------------------------------------------------------
# Apply Page Review Notification API
# Returns full approval/refusal/message history for one application.
# ---------------------------------------------------------------------

def _apply_notify_row_to_dict(row):
    try:
...
def _apply_notify_get(row, names, default=None):
    for name in names:
...
def _apply_notify_status(value):
    text_value = str(value or "").strip().lower()
    ...
    if (
        text_value == "approved"
        or text_value == "approve"
        or text_value == "accepted"
        or "approved" in text_value
    ):
        return "approved"

    if (
        text_value == "refused"
        or text_value == "refuse"
        or text_value == "rejected"
        or text_value == "reject"
        or text_value == "declined"
        or text_value == "denied"
...
def _apply_notify_sort_key(item):
    submission_id = _apply_notify_int(item.get("submission_id") or item.get("id"))

    date_value = str(
        item.get("decision_at")
    ...
    date_digits = "".join(ch for ch in date_value if ch.isdigit())

    try:
        date_score = int(date_digits[-14:]) if date_digits else 0
    except Exception:
        date_score = 0

    return (submission_id, date_score)


def _apply_notify_normalize_row(row, application_id):
    submission_id = _apply_notify_int(
        _apply_notify_get(
            row,
            ["submission_id", "review_submission_id", "id"],
            0,
        )
    )

    raw_status = _apply_notify_get(
        row,
        [
            "decision",
            "decision_status",
    ...
    status = _apply_notify_status(raw_status)
...
def _apply_notify_find_application_rows(db, application_id):
    ...
    for table_name in table_names:
        ...
        if not (
            ...
            or table_lower in ["applications", "loan_applications"]
        ):
            continue
        ...
        app_columns = [
            ...
            if column.lower() in ["application_id", "app_id", "loan_application_id"]
        ]
        ...
        for app_column in app_columns:
            try:
                rows = db.execute(
                    text(f'SELECT * FROM "{table_name}" WHERE "{app_column}" = :application_id'),
            ...
            for row in rows:
                row_dict = _apply_notify_row_to_dict(row)
                row_dict["_source_table"] = table_name
                candidates.append(row_dict)

    normalized = [
        _apply_notify_normalize_row(item, app_id)
        for item in candidates
    ]

    # Remove duplicates by submission id/status/time/message.
    seen = set()
    unique = []
    ...
    unique.sort(key=_apply_notify_sort_key, reverse=True)

    return unique


@router.get("/applications/{application_id}/notifications")
def get_apply_page_review_notifications(
    application_id: int,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    ...
    notifications = _apply_notify_find_application_rows(db, application_id)
...
def resolve_document_file_path_for_review(document: ApplicationDocument | None) -> Path | None:
    if not document:
        return None
...
def is_loan_application_pdf_document(document: ApplicationDocument) -> bool:
    text_value = document_text(document)
...
def get_review_loan_application_document(
    db: Session,
    submission: dict,
    ...
    document_ids = document_ids_from_submission(submission)
    snapshot_documents = documents_by_ids(db, document_ids)

    for document in snapshot_documents:
        if is_loan_application_pdf_document(document):
            return document
    ...
    for document in latest_documents:
        path = resolve_document_file_path_for_review(document)
...
@router.get("/submissions/{submission_id}/loan-application-pdf")
def view_review_loan_application_pdf(
    submission_id: int,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    ...
    submission = get_submission_or_404(db, submission_id)
    document = get_review_loan_application_document(db, submission)

    if not document:
        raise HTTPException(status_code=404, detail="Loan Application PDF document not found for this submission")

    file_path = resolve_document_file_path_for_review(document)

    if not file_path or not file_path.exists():
        raise HTTPException(status_code=404, detail="Loan Application PDF file missing from storage")

    filename = (
        getattr(document, "original_file_name", None)
        or getattr(document, "file_name", None)
        or getattr(document, "stored_file_name", None)
        or f"loan_application_submission_{submission_id}.pdf"
    )
