...
def mlops_view_safe_params(value, max_items: int = 35) -> dict:
    try:
        if not isinstance(value, dict):
            return {}
...
def mlops_extract_missing_module(error_text: str) -> str | None:
    import re
...
def mlops_install_pickle_compat_aliases() -> list[str]:
    import importlib
    import sys
    ...
    aliases = {
        ...
        "_loss.glm_distribution": "sklearn._loss.glm_distribution",
    }
    ...
    for alias, target in aliases.items():
        try:
            ...
            installed.append(f"{alias} -> {target}")
        except Exception:
            pass
...
def mlops_scan_serialized_file(path: Path) -> dict:
    import hashlib
    import re
    ...
    try:
        ...
        return {
            "sha256": "",
            "detected_modules": [],
            "detected_warning": mlops_view_safe_string(error, 300),
        }

    sha256 = hashlib.sha256(data).hexdigest()

    # This does not execute the pickle. It only scans readable module-like strings.
    text = data[: min(len(data), 8 * 1024 * 1024)].decode("latin1", errors="ignore")

    patterns = [
        r"sklearn(?:[._][A-Za-z0-9_]+)+",
        r"numpy(?:[._][A-Za-z0-9_]+)*",
        r"scipy(?:[._][A-Za-z0-9_]+)*",
        r"pandas(?:[._][A-Za-z0-9_]+)*",
        r"xgboost(?:[._][A-Za-z0-9_]+)*",
        r"lightgbm(?:[._][A-Za-z0-9_]+)*",
        r"catboost(?:[._][A-Za-z0-9_]+)*",
        r"imblearn(?:[._][A-Za-z0-9_]+)*",
        r"_loss(?:[._][A-Za-z0-9_]+)*",
    ]
    ...
    for pattern in patterns:
        for item in re.findall(pattern, text):
            cleaned = item.strip(" .,\x00\r\n\t")
            if len(cleaned) >= 2:
                detected.add(cleaned)

    return {
        "sha256": sha256,
        "detected_modules": sorted(detected)[:80],
        "detected_warning": "",
    }
...
def mlops_view_load_model(path: Path):
    errors = []
    aliases = mlops_install_pickle_compat_aliases()

    try:
        import joblib

        return joblib.load(path), "joblib", errors, aliases
    except Exception as error:
        errors.append(f"joblib load failed: {mlops_view_safe_string(error, 500)}")

    try:
        import pickle

        with path.open("rb") as file:
            return pickle.load(file), "pickle", errors, aliases
    except Exception as error:
        errors.append(f"pickle load failed: {mlops_view_safe_string(error, 500)}")

    return None, "metadata_only", errors, aliases
...
def mlops_view_sample_prediction(model_object, feature_names: list[str], classes: list[str]) -> dict:
    ...
    try:
        ...
        return {
            ...
            "sample_prediction": None,
            "sample_probabilities": {},
            "sample_error": mlops_view_safe_string(error, 500),
        }
...
def inspect_model_file_perfect(path: Path, limit: int = 10) -> dict:
    suffix = path.suffix.lower()
    file_size_mb = round(path.stat().st_size / (1024 * 1024), 4)
    serialized_scan = mlops_scan_serialized_file(path)
    ...
    model_object, loader, load_errors, aliases = mlops_view_load_model(path)

    if model_object is None:
        error_text = " | ".join(load_errors)
        missing_module = mlops_extract_missing_module(error_text)

        suggestions = [
            "This model file is stored successfully.",
            "To fully load it, use the same Python/scikit-learn dependency versions used during training.",
            "Best production fix: re-export this model inside the current SmartLoan backend environment.",
        ]

        if missing_module:
            suggestions.insert(1, f"Missing module detected: {missing_module}")

        return {
            "preview_type": "model_file_view",
            "view_mode": "dependency_error",
            "loader": loader,
            "object_type": "Serialized binary model file",
            "object_module": "unknown",
            "can_predict": False,
            "can_predict_proba": False,
            "classes": [],
            ...
            "file_extension": suffix,
            "file_size_mb": file_size_mb,
            "sha256": serialized_scan.get("sha256", ""),
            "detected_modules": serialized_scan.get("detected_modules", []),
            "missing_module": missing_module or "",
            "compat_aliases_attempted": aliases,
            "dependency_suggestions": suggestions,
            "load_status": "not_loaded_dependency_error",
            "raw_file_notice": raw_file_notice,
            "compatibility_note": "The file exists, but it references a dependency/module that is not available in this backend environment.",
            "load_errors": load_errors,
            "message": "Model file stored successfully. Full preview is blocked by missing dependency.",
        }
