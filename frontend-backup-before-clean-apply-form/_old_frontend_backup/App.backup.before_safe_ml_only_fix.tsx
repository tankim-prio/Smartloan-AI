...
function MLModelPage() {
  ...
  const localPercent = (value?: number | null) => {
  ...
  return (
    <AppLayout>
      ...
      <div className="card">
        ...
        {activeModel ? (
          <div className="grid two">
            <div>
              <div className="field-row"><strong>Model Name</strong><span>{localText(activeModel.model_name)}</span></div>
              <div className="field-row"><strong>Version</strong><span>{localText(activeModel.version)}</span></div>
              <div className="field-row"><strong>Type</strong><span>{localText(activeModel.model_type)}</span></div>
              <div className="field-row"><strong>Status</strong><span>{localText(activeModel.status)}</span></div>
            </div>

            <div>
              <div className="field-row"><strong>Accuracy</strong><span>{localPercent(activeModel.accuracy)}</span></div>
              <div className="field-row"><strong>Precision</strong><span>{localPercent(activeModel.precision)}</span></div>
              <div className="field-row"><strong>Recall</strong><span>{localPercent(activeModel.recall)}</span></div>
              <div className="field-row"><strong>F1 Score</strong><span>{localPercent(activeModel.f1_score)}</span></div>
            </div>
          </div>
        ) : (
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card"><p>High Risk</p><h2>{highRiskCount}</h2></div>
      </div>
      ...
      <div className="card">
        ...
        <div className="grid two">
          <div className="field-row"><strong>Low Risk</strong><span>{lowRiskCount}</span></div>
          <div className="field-row"><strong>Medium Risk</strong><span>{mediumRiskCount}</span></div>
          <div className="field-row"><strong>High Risk</strong><span>{highRiskCount}</span></div>
        </div>
      </div>
          ...
          <table>
            ...
            <tbody>
              {models.map((model) => (
                <tr key={model.id}>
                  <td>#{model.id}</td>
                  <td><strong>{localText(model.model_name)}</strong><br /><small>{localText(model.version)}</small></td>
                  <td>{localText(model.model_type)}</td>
                  <td>{localText(model.status)}</td>
                  <td>{model.is_active ? "Yes" : "No"}</td>
                  <td>{localPercent(model.accuracy)}</td>
                  <td>{localPercent(model.f1_score)}</td>
                  <td>
                    <div className="actions">
                      <button className="btn secondary" onClick={() => deployModel(model.id)}>Deploy</button>
                      <button className="btn success" onClick={() => setModelActive(model.id)}>Set Active</button>
              ...
              {applications.map((application) => {
                ...
                return (
                  <tr key={application.id}>
                    ...
                    <td>{localText(application.occupation)}</td>
                    <td>{localText(application.monthly_income)}</td>
                    <td>{localText(application.status)}</td>
                    <td>{ready ? <span className="badge badge-approved">Ready</span> : <span className="badge badge-rejected">Step 2 Required</span>}</td>
                    <td>
                      {latestPrediction ? (
                        <div>
                          <strong>{latestPrediction.prediction_result.replaceAll("_", " ")}</strong><br />
                          <span>{latestPrediction.risk_level}</span><br />
                          <small>{localPercent(latestPrediction.confidence_score)}</small>
                        </div>
                      ) : (
            ...
            <tbody>
              {predictions.map((prediction) => (
                <tr key={prediction.id}>
                  ...
                  <td>{localPercent(prediction.confidence_score)}</td>
                  <td>{prediction.reason}</td>
                  <td>{localDate(prediction.created_at)}</td>
                </tr>
              ))}
