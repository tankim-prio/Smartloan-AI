...
from fastapi import APIRouter, Body, Depends, HTTPException, File, Form, UploadFile
...
def save_supporting_upload_file(
    db: Session,
    application: Application,
    upload_file: UploadFile,
    document_type: str,
):
    upload_dir = Path("uploads") / "applications" / str(application.id)
    ...
    stored_name = f"{safe_document_type}_{uuid.uuid4().hex}{suffix}"
    ...
    if "stored_file_path" in columns:
        data["stored_file_path"] = str(file_path)
    ...
    if "content_type" in columns:
        data["content_type"] = upload_file.content_type or "application/octet-stream"

    if "mime_type" in columns:
        data["mime_type"] = upload_file.content_type or "application/octet-stream"
...
@router.post("/applications/{application_id}/upload-supporting-documents")
def upload_supporting_documents_for_pdf(
    application_id: int,
    photo: UploadFile | None = File(default=None),
    income_document: UploadFile | None = File(default=None),
    identity_document: UploadFile | None = File(default=None),
    income_document_type: str = Form(default="salary_certificate"),
    identity_document_type: str = Form(default="nid"),
    ...
    if income_document is not None:
        if income_document_type not in ["salary_certificate", "tin_certificate"]:
            income_document_type = "salary_certificate"

        saved_documents.append(
            save_supporting_upload_file(
                db=db,
                application=application,
                upload_file=income_document,
                document_type=income_document_type,
            )
        )

    if identity_document is not None:
        if identity_document_type not in ["nid", "passport"]:
            identity_document_type = "nid"

        saved_documents.append(
            save_supporting_upload_file(
                db=db,
                application=application,
                upload_file=identity_document,
