...
def generate_application_pdf_route(
    ...
    if "application_id" in columns:
        document_data["application_id"] = application.id

    if "user_id" in columns:
        document_data["user_id"] = application.user_id

    if "document_type" in columns:
        document_data["document_type"] = "generated_pdf"

    if "original_file_name" in columns:
        document_data["original_file_name"] = file_name

    if "file_name" in columns:
        document_data["file_name"] = file_name

    if "stored_file_name" in columns:
        document_data["stored_file_name"] = Path(pdf_path).name

    if "file_path" in columns:
        document_data["file_path"] = pdf_path

    if "stored_file_path" in columns:
        document_data["stored_file_path"] = pdf_path

    if "file_size" in columns:
        document_data["file_size"] = file_size

    if "size_bytes" in columns:
        document_data["size_bytes"] = file_size

    if "content_type" in columns:
        document_data["content_type"] = "application/pdf"

    if "mime_type" in columns:
        document_data["mime_type"] = "application/pdf"

    document = ApplicationDocument(**document_data)
