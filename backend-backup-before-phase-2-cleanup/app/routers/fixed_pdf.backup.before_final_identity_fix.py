...
def create_full_fixed_pdf(db: Session, application: Application) -> Path:
    ...
    # Safety: never repeat the same physical file as both Salary and NID/Passport.
    # If identity accidentally points to the same salary file, page 3 becomes a clean placeholder.
    ...
    if income_path and income_path.exists():
        append_pdf_or_image(
            writer,
            "Salary Certificate / TIN Certificate",
            income_path,
        )
    else:
        missing_income_page = create_missing_document_page(
            "Salary Certificate / TIN Certificate",
            "Salary Certificate or TIN Certificate was not uploaded for this application.",
        )

        try:
            missing_reader = PdfReader(str(missing_income_page))
            for page in missing_reader.pages:
                writer.add_page(page)
        finally:
            try:
                missing_income_page.unlink(missing_ok=True)
            except Exception:
                pass
    ...
    if identity_path and identity_path.exists():
        ...
        missing_identity_page = create_missing_document_page(
            "NID / Passport Document",
            "NID or Passport document was not uploaded for this application.",
        )

        try:
            missing_reader = PdfReader(str(missing_identity_page))
            for page in missing_reader.pages:
                writer.add_page(page)
        finally:
            try:
                missing_identity_page.unlink(missing_ok=True)
            except Exception:
                pass
