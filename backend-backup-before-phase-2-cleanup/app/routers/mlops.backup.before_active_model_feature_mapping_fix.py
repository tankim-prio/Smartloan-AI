...
def try_uploaded_model_prediction(model_path: str | None, features: dict) -> dict | None:
    ...
    try:
        ...
        predict_function = getattr(model_object, "predict", None)
        ...
        raw_prediction = predict_function(x)[0]
        ...
        if callable(predict_proba):
            try:
                proba = predict_proba(x)[0]
                probability = float(max(list(proba))) * 100
            except Exception:
                probability = None

        prediction_text = str(raw_prediction).lower()

        if prediction_text in ["1", "approved", "approve", "recommended_approval", "low", "true"]:
            return {
                "result": "recommended_approval",
                "risk_level": "low",
                "confidence": probability or 85,
                "reason": f"Prediction generated by active uploaded model using {loader}.",
            }
...
def get_model_preview(
    ...
    preview = inspect_model_file(file_path)
