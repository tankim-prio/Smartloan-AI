...
def ensure_review_table(db: Session) -> None:
    ...
    # Add missing columns safely for older existing table.
    try:
...
def create_review_submission(db: Session, application: Application) -> dict:
    ensure_review_table(db)

    created_at = now_text()
    snapshot = serialize_application_live(application)
